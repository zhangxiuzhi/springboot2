...
var UserTable = function (_React$Component) {
	...
	function UserTable(props) {
		...
		_this.thead = [{ dataField: "id", dataName: "操作", width: 90, dataFormat: formatOperation }, { dataField: "merchantName", dataName: "商户名称", width: 110 }, { dataField: "userName", dataName: "操作员", width: 120 }, { dataField: "phone", dataName: "手机号", width: 120 }];
		_this.state = {
			data: [],
			page: 1,
			size: 10,
	...
	_createClass(UserTable, [{
		...
		value: function render() {
			...
			var options = {
				url: "/merchantUser/listData", //数据请求url
				thead: this.thead, //表头
...
function formatOperation(cell) {
	var html = "";
	html += "<a href='/merchantUser/edit/" + cell + "' class='ui-link-icon fal fa-pencil-alt ' data-tabname='" + cell + "' data-tabtype='edit' data-tabtitle='商户管理员编辑(" + cell + ")' title='编辑'></a>";
	html += "<a href='javascript:void(0)' action='/merchantUser/delete/" + cell + "' onclick='del_rowData(this)' class='ui-link-icon far fa-trash-alt' title='删除'></a>";
	return html;
}
	...
	confirm(null, "是否删除？", function () {
		jbsframe.ajaxRequest({
			...
			notification("删除成功", function () {
				tps_merchantUser.table.reloadTable();
			});
		});
	});
}